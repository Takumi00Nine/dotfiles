...
TARGET="$SCRIPT_DIR/../cmux-task-declare.sh"
WATCH_TARGET="$SCRIPT_DIR/../cmux-task-watch.sh"
...
# AC-41/AC-43/DT-5 が cmux-task-watch.sh を直接呼ぶ箇所も同じ前後差分検査
# の対象にする（D+U複合ACなので、こちらのファイルでも$VAULTの不変を見る）。
run_watch_plain() {
  local before after out
  before="$(vault_snapshot)"
  out="$(CMUX_TASK_VAULT="$VAULT" CMUX_TASK_STATE="$STATE_FILE" CMUX_TASK_COLS=40 CMUX_TASK_ROWS=24 \
    bash "$WATCH_TARGET" --plain --once)"
  after="$(vault_snapshot)"
